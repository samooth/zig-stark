...
/// Prove and verify one batch of 4-bit additions over `F`, reporting wall-clock
/// prove/verify times and the in-memory proof-size breakdown. Eval openings
/// open every hypercube entry (no low-degree test), so they dominate at large
/// k — the benchmark makes that O(2^k) blow-up explicit.
fn runField(comptime F: type, alloc: std.mem.Allocator, rnd: std.Random) !void {
...
pub fn main() !void {
    ...
    try runField(Gf16, alloc, rnd);
    try runField(Gf256, alloc, rnd);
}
